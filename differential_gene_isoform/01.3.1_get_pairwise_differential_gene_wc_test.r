...
gene_tpm = fread(file.path(exp_dir, "7_stage_gene_TPM.txt"),sep="\t",header=T,data.table=F)
tpm_mat = gene_tpm[,-1]
row.names(tpm_mat) = gene_tpm$gene_id
...
getSigGenes <- function(stage1, stage2, outpath = outPath, cutoff = 0.05, gene_tpm = tpm_mat, sampleStage = sample2stage, GeneInfor = gene_infor){
...
DE_gene_frm = data.frame(gene_id = intersect_gene,
GeneInfor[intersect_gene,],
DE_gene[intersect_gene,], check.rows=T,check.names=F)
colnames(DE_gene_frm)[4:5] = paste(c(stage1,stage2),'median_TPM',sep='_')
...
outPath = "./diff_res/gene_level/pairwise_test"
