...
transcript_tpm_median = t(apply(transcript_tpm[,samples],1,function(x){tapply(x,as.character(sample2stage[names(x),"stage"]),median)}))
transcript_tpm_median_high = transcript_tpm_median[apply(transcript_tpm_median,1,function(x){length(x[x>1])>0}),]
transcript_tpm_filter_tab = data.frame(transcript_infor[row.names(transcript_tpm_median_high),],transcript_tpm_median_high,check.names=F)

all_sig_iso_tab = read.table("./diff_iso_and_gene/AEC_HEC_T1/AEC_HEC_T1_pre_HSC_diff_iso_and_gene_merge.txt",sep="\t",header=T,check.names=F)
...
#plot candidates 
########################################
candi_gene = c("Runx1","Dnmt3b","Hlf")
#"Myb" "Gfi1" "Hlf" "Gata2" "Sox17"
#------get sig isoform.
gene = "Dnmt3b"
candidates_frm = all_sig_iso_tab[all_sig_iso_tab$gene_name == gene,]
candidates_frm
candidates_all_tab = transcript_tpm_filter_tab[transcript_tpm_filter_tab$gene_name==gene,]
candidate_gene_id = unique(as.character(candidates_all_tab$gene_id))
sig_iso = unique(as.character(candidates_frm[!is.na(candidates_frm$enrich_type),"transcript_id"]))
not_sig_iso = setdiff(row.names(candidates_all_tab),sig_iso)
...
plot_frm$sample = factor(plot_frm$sample, levels = sample_ordered)
plot_frm$gene_name = factor(candi_gene_frm[plot_frm$gene_id,"gene_name"], levels = candi_gene)
#color_fill = c("#2080DF","#f4606c")
#color_fill = c("#DF2080")
...
p = ggplot(plot_frm, aes(x = sample, y = log2(TPM+1), fill= gene_name)) + 
geom_bar(stat = "identity",position = "stack",size=.1,width=.8, color=NA)+
scale_y_continuous(limits=c(0,max),breaks = seq(0,max,max/2))+
#scale_fill_manual(values = color_fill)+
theme(line = element_line(size=line_size), text = element_text(size=text_size, color="black"), 
