...
WT_KO_counts = read.table(paste(indir,"gene_raw_counts.txt",sep="/"),sep="\t",header=T,row.names=1,check.names=F)
...
gene_median_tpm = t(apply(gene_tpm,1,function(x){tapply(x,EC_condition,median)}))
colnames(gene_median_tpm) = paste(colnames(gene_median_tpm),"_median_TPM",sep="")

EC_de_res = doedgeR(x = EC_WT_KO_exp, group = EC_condition)
EC_inter_gene = intersect(row.names(EC_de_res),row.names(gene_infor))
EC_de_frm = data.frame(gene_infor[EC_inter_gene,],gene_median_tpm[EC_inter_gene,],EC_de_res[EC_inter_gene,],check.rows=T)
outdir = "./diff_res"
dir.create(outdir, recursive = T)
write.table(EC_de_frm,paste(outdir,"EC_WT_KO_edgeR_test_res_all.txt",sep="/"),sep="\t",quote=F)
